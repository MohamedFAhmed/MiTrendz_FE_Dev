    ...
    cloud.start = function() {
      ...
      function step() {
        ...
        while (+new Date - start < timeInterval && ++i < n && timer) {
          d = data[i];
          d.x = (size[0] ) >> 1; //* (Math.random() + .5)
          d.y = (size[1] ) >> 1; //* (Math.random() + .5)
          cloudSprite(d, data, i);
          if (place(board, d, bounds)) {
            tags.push(d);
            event.word(d, creatorRef);
        ...
        if (i >= n) {
          cloud.stop();
          event.end(tags,creatorRef, bounds);
        }
      }
    }
  ...
  function cloudSprite(d, data, di) {
    ...
    while (++di < n) {
      ...
      if (d.rotate) {
        ...
        h = ~~Math.max(Math.abs(wsr + hcr), Math.abs(wsr - hcr)) + 10;
      } else {
        w = (w + 0x1f) >> 5 << 5;
      }
      if (h > maxh) maxh = h;
